...
const products = [
  {
    ...
    btnColor: "bg-[#d39b16] hover:bg-[#b88610] shadow-[0_4px_15px_rgba(211,155,22,0.4)] hover:shadow-[0_6px_20px_rgba(211,155,22,0.6)]"
  },
  {
    name: "Raw Peanuts",
    title1: "RAW",
    ...
    btnColor: "bg-[#496506] hover:bg-[#3d5405] shadow-[0_4px_15px_rgba(73,101,6,0.4)] hover:shadow-[0_6px_20px_rgba(73,101,6,0.6)]"
  },
  {
    name: "Chickpeas",
    title1: "ROASTED",
...
const floatingNuts = [
  { src: imgObject.src, className: "w-16 md:w-20 top-[10%] left-[8%] rotate-[45deg]", delay: 0 },
  { src: imgObject1.src, className: "w-12 md:w-16 top-[20%] right-[12%] -rotate-[30deg]", delay: 1 },
  { src: imgObject.src, className: "w-14 md:w-24 bottom-[25%] left-[12%] rotate-[120deg]", delay: 2 },
  { src: imgObject1.src, className: "w-10 md:w-14 bottom-[30%] right-[10%] -rotate-[15deg]", delay: 1.5 },
];
  ...
  const handleScroll = () => {
    if (scrollRef.current) {
      ...
      if (maxScroll > 0) {
        ...
        // Map percentage to 3 dots (0, 1, 2)
        const dotIndex = Math.min(2, Math.floor(percentage * 3));
        setCurrentDot(dotIndex);
      }
    }
  };

  // Sync currentDot with activeIndex if user clicks
  useEffect(() => {
    setCurrentDot(Math.floor(activeIndex / 2));
  }, [activeIndex]);

  return (
    <section id="products" className="relative w-full min-h-screen bg-white overflow-hidden py-16 lg:py-24 flex flex-col justify-center font-['Roboto',sans-serif]">
      {/* Background Glow */}
      ...
      {floatingNuts.map((nut, i) => (
        <motion.img
          ...
          transition={{ duration: 5, repeat: Infinity, ease: "easeInOut", delay: nut.delay }}
        />
      ))}
      ...
      <div className="container mx-auto px-4 relative z-10 flex-grow flex flex-col justify-center max-w-6xl">
        ...
        <div className="w-full flex flex-col lg:flex-row items-center justify-center gap-12 lg:gap-24 flex-grow mb-8 lg:mb-16">
          ...
          <div className="w-full lg:w-[45%] flex justify-center lg:justify-end relative">
            <div className="relative w-full max-w-[280px] sm:max-w-[320px] md:max-w-[374px]" style={{ aspectRatio: '374/402' }}>
          ...
          <div className="w-full lg:w-[55%] flex flex-col items-center lg:items-start text-center lg:text-left relative min-h-[300px]">
            <AnimatePresence mode="wait">
              ...
              >
                ...
                {/* Button */}
                <Link href="/shop">
                  <button className={`text-white px-8 md:px-10 py-3 rounded-[25px] font-medium text-[15px] md:text-[16px] transition-all duration-300 ${products[activeIndex].btnColor}`}>
                    Shop Now
                  </button>
                </Link>
              </motion.div>
        ...
        <div className="w-full flex justify-center mt-auto z-20">
          <div 
            ...
            className="flex gap-3 sm:gap-4 md:gap-6 overflow-x-auto pb-6 pt-4 px-4 snap-x snap-mandatory w-full max-w-5xl justify-start lg:justify-center [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
              ...
              return (
                ...
                >
                  <div className="relative w-[65px] sm:w-[80px] md:w-[100px]" style={{ aspectRatio: '100/108' }}>
                    <div className={`absolute inset-0 overflow-hidden pointer-events-none transition-transform duration-300 transform ${isActive ? 'scale-110' : 'group-hover:scale-110'}`}>
                      <img
                        src={imgVarietyNutsBowls1.src}
                        alt={prod.name}
                        className="absolute max-w-none"
                  ...
                  <span
                    className={`text-[11px] sm:text-[13px] md:text-[14px] font-medium text-center leading-[1.2] transition-colors ${isActive ? 'text-[#373737]' : 'text-[#8c8b8a] group-hover:text-[#373737]'
                      }`}
                  >
