...
const floatingNuts = [
  ...
  { src: imgObject2, className: "w-20 md:w-24 top-[20%] right-[15%] -rotate-[30deg]", delay: 1 },
  { src: imgObject, className: "w-12 md:w-16 bottom-[25%] left-[25%] rotate-[45deg]", delay: 2 },
  { src: imgObject2, className: "w-16 md:w-24 bottom-[20%] right-[30%] rotate-[120deg]", delay: 1.5 },
  { src: imgObject, className: "w-14 md:w-20 top-[45%] left-[30%] rotate-[15deg]", delay: 0.5 },
  { src: imgObject2, className: "w-14 md:w-24 top-[50%] right-[35%] -rotate-[45deg]", delay: 2.5 },
];
        ...
        <div className="hidden lg:block absolute inset-0 w-full h-full mt-24">
          {features.map((feature, idx) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className={`absolute ${feature.desktopPos} flex items-center gap-6 ${feature.align === 'left' ? 'flex-row-reverse' : 'flex-row'} w-max max-w-[450px] xl:max-w-[500px] group justify-end`}
            >
              <p className={`text-[#373737] text-[15px] leading-[20px] w-[180px] xl:w-[280px] ${feature.align === 'left' ? 'text-left' : 'text-right'} transition-colors duration-300 group-hover:text-black`}>
                {feature.description}
              </p>
